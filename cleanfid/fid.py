...
def get_folder_features(files, model=None, num_workers=12, num=None,
...
def compare_folders(files1, files2, feat_model, mode, num_workers=0,
                    batch_size=8, device=torch.device("cuda")):
    # get all inception features for the first folder
    # fbname1 = os.path.basename(fdir1)
    print('get dir1 features...')
    np_feats1 = get_folder_features(files1, feat_model, num_workers=num_workers,
                                    batch_size=batch_size, device=device, mode=mode)
    mu1 = np.mean(np_feats1, axis=0)
    sigma1 = np.cov(np_feats1, rowvar=False)

    # get all inception features for the second folder
    print('get dir2 features...')
    # fbname2 = os.path.basename(fdir2)
    np_feats2 = get_folder_features(files2, feat_model, num_workers=num_workers,
                                    batch_size=batch_size, device=device, mode=mode)

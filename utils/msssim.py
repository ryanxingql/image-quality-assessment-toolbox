...
# All rights reserved.
# ref: https://github.com/VainF/pytorch-msssim/blob/master/pytorch_msssim/ssim.py
#
import torch
...
class MSSSIM:
    def __init__(self):
        ...
        self.nchannel = 3
        self.spatial_dims = 2
        self.win = _fspecial_gauss_1d(self.win_size, self.win_sigma).repeat([self.nchannel, 1] + [1] * self.spatial_dims)
        self.data_range = 255
    ...
    def _preprocess(self, img):
        img = img[:, :, ::-1].copy()  # (H W BGR) -> (H W RGB)
    ...
    def forward(self, img1, img2):
        ...
        if self.win is None:
            win = _fspecial_gauss_1d(win_size, self.win_sigma)
        ...
        for i in range(levels):
            ssim_per_channel, cs = _ssim(img1, img2, win=win, data_range=self.data_range, size_average=False, K=self.K)
        ...
        if self.size_average:
            ...
            return ms_ssim_val.mean(1).item()
